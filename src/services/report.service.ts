...
export class ReportService {
	...
	async getByReporterId(reporterId: string): Promise<InstanceType<typeof Report>[]> {
		try {
	...
	async updateStatus(
        ...
        try {
          ...
          logger.info(`Successfully updated status of report ${id} to ${status}`);
	...
	async action(id: string, action: actionReport): Promise<InstanceType<typeof Report> | null> {
		try {
			if (!Types.ObjectId.isValid(id)) {
			...
			logger.info(`Successfully updated action of user ${reportedUserId} to ${action}`);
			logger.info(`Successfully updated status of report ${id} to RESOLVED`);
			return updatedReport;
		} catch (error) {
